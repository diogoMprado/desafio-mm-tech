...
app.post('/funcionarios', (req, res) => {
    ...
    db.insert(funcionario, (err, newDoc) => {
        ...
        return res.status(201).json(newDoc).send('Funcionário cadastrado com sucesso');
    });
});
app.get('/funcionarios', (req, res) => {
    db.find({}, (err, docs) => {
        ...
        return res.status(200).json(docs).send('Funcionários listados com sucesso');
    });
});
app.get('/funcionarios/:id', (req, res) => {
    ...
    db.findOne({_id: id}, (err, doc) => {
        if (err) {
            ...
            res.status(200).json(doc).send('Funcionário encontrado com sucesso');
        }
    });
});
